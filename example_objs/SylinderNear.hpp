...
//  public:
//    std::shared_ptr<CollisionBlockPool> colPoolPtr; ///< shared object for
//    collecting collision constraints
...
//#ifdef DEBUGLCPCOL
//        std::cout << "stress recoder size:" << colPoolPtr->size() <<
//        std::endl;
//#endif
//    }
 ...
 */
//    void operator()(const SylinderNearEP *const ep_i, const PS::S32 Nip, const
//    SylinderNearEP *const ep_j,
...
//            const Evec3 Pm = centerI - directionI * (0.5 *
//            syI.lengthCollision); // minus end const Evec3 Pp = centerI +
//            directionI * (0.5 * syI.lengthCollision); // plus end

//            for (PS::S32 j = 0; j < Njp; ++j) {
//                auto &syJ = ep_j[j];
//                const Evec3 centerJ = ECmap3(syJ.pos);
//                const Evec3 directionJ = ECmap3(syJ.direction);
//                const Evec3 Qm = centerJ - directionJ * (0.5 *
//                syJ.lengthCollision); // minus end const Evec3 Qp = centerJ +
//                directionJ * (0.5 * syJ.lengthCollision); // plus end

//                Evec3 Ploc = Evec3::Zero();
//                Evec3 Qloc = Evec3::Zero();
//                double s, t = 0;
//                const double distMin = DistSegSeg3(Pm, Pp, Qm, Qp, Ploc, Qloc,
//                s, t); forceI.sepmin = std::min(forceI.sepmin, distMin);

//                // add to collision processing list
//                // save collision block
//                // save only block gidI < gidJ, and gid
//                const double sep = distMin - (syI.radiusCollision +
//                syJ.radiusCollision); // target is sep >=0

//                // record collision blocks
//                if (sep < COLBUF * (syI.radiusCollision + syJ.radiusCollision)
//                && syI.gid < syJ.gid) {
//                    // in this constructor need conversion between PS::F64vec3
//                    and Eigen::Vector3d const double phi0 = sep; const double
//                    gamma = sep < 0 ? -sep : 0; const Evec3 PlocEvec(Ploc[0],
//                    Ploc[1], Ploc[2]); const Evec3 QlocEvec(Qloc[0], Qloc[1],
//                    Qloc[2]); const Evec3 normI = (PlocEvec -
//                    QlocEvec).normalized(); const Evec3 normJ = -normI; const
//                    Evec3 posI = Ploc - centerI; const Evec3 posJ = Qloc -
//                    centerJ;
//                    (*colPoolPtr)[myThreadId].emplace_back(phi0, gamma,
//                    syI.gid, syJ.gid, syI.globalIndex,
//                                                           syJ.globalIndex,
//                                                           normI, normJ, posI,
//                                                           posJ, false);
//                    Emat3 &stressIJ = (*colPoolPtr)[myThreadId].back().stress;
//                    collideStress(directionI, directionJ, centerI, centerJ,
//                    syI.lengthCollision, syJ.lengthCollision,
//                                  syI.radiusCollision,
//                                  syJ.radiusCollision, 1.0, PlocEvec,
//                                  QlocEvec, stressIJ);
//                }
//            }
//        }
 ...
 * @param StressIJ
 */
//    void collideStress(const Evec3 &dirI, const Evec3 &dirJ, const Evec3
//    &posI, const Evec3 &posJ, double hI, double hJ,
//                       const double rI, const double rJ, const double rho,
...
//        InvGAMMAI = aI * Emat3::Identity() + (bI - aI) * (dirI *
//        dirI.transpose()); InvGAMMAJ = aJ * Emat3::Identity() + (bJ - aJ) *
//        (dirJ * dirJ.transpose());
...
//                    for (int l = 0; l < 3; l++) {
//                        for (int r = 0; r < 3; r++) {
//                            SGI(i, j) = SGI(i, j) + NI(i, l) *
//                            epsilon[j][k][l] * InvGAMMAI(k, r) * xICf(r);
//                            SGJ(i, j) = SGJ(i, j) + NJ(i, l) *
//                            epsilon[j][k][l] * InvGAMMAJ(k, r) * xJCf(r);
//                        }
//                    }
//                }
//            }
...
//        NI(1, 1) = NI(0, 0);
//        NI(2, 2) = 1.0 / 15.0 * (10.0 * beta * beta * beta + 20.0 * beta *
//        beta + 15.0 * beta + 4.0); NI = NI * rho * r * r * r * r * r * M_PI;
//    }

//    void InitializeSyGA(Emat3 &GAMMAI, double r, double h, double rho) const {
//        GAMMAI.setZero();
//        double beta = h / 2.0 / r;
//        GAMMAI(0, 0) = 1.0 / 30.0 * (20.0 * beta * beta * beta + 40.0 * beta *
//        beta + 45.0 * beta + 16.0); GAMMAI(1, 1) = GAMMAI(0, 0); GAMMAI(2, 2)
//        = 1.0 / 15.0 * (15 * beta + 8); GAMMAI = GAMMAI * M_PI * r * r * r * r
//        * r * rho;
//    }
//};
 ...
 *
 */
// using TreeSylinderNear = PS::TreeForForceShort<ForceNear, SylinderNearEP,
// SylinderNearEP>::Symmetry;
