...
class KMC {
  private:
    ...
    double prob_tot_;
    double r_cutoff_;
    std::vector<double> ep_j_probs_; ///< binding probabilities

    // Spatial variables
    double pos_[3];                   ///< position of motor head
    std::vector<double> distMinArr_;  ///< min dist to tubule segment
    std::vector<double> distPerpArr_; ///< min (perp) dist to tubule line
  ...
  public:
    ...
    void CalcTotProbsUS(const SylinderNearEP *const *ep_j,
                        const std::vector<int> &uniqueFlagJ,
                        const double bindFactor);

    double CalcProbUS(const int j_bond, const SylinderNearEP &tubule,
                      const double bindFactor);

    void CalcProbSU(const double unbindFactor);

    void CalcTotProbsSD(const SylinderNearEP *const *ep_j,
    ...
    double LUCalcProbSD(const int j_rod, const SylinderNearEP &rod,
                        const double bindFactor);

    double CalcProbSD(const int j_rod, const SylinderNearEP &rod,
                      const double lambda, const double kappa,
                      const double beta, const double restLen,
                      const double bindFactor);
    ...
    void UpdateRodDistArr(const int j_bond, const SylinderNearEP &tubule);

    double getTotProb() { return prob_tot_; }

    int whichRodBindUS(const SylinderNearEP *const *ep_j, double &bindPos,
                       double roll);
    ...
    double getProbs(const int j_bond) { return ep_j_probs_[j_bond]; }
